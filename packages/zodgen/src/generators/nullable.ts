...
export const generateNullable = <T>(ctx: GenContext<T | null, 'nullable'>): unknown => {
  ...
  return ctx.faker.number.float() < 1 - ctx.config.nullRate ? ctx.generate(innerType as z.ZodType) : null;
};

export const generateOptional = <T>(ctx: GenContext<T | undefined, 'optional'>): unknown => {
  const { innerType } = ctx.def;
  return ctx.faker.number.float() < ctx.config.optionalRate ? ctx.generate(innerType as z.ZodType) : undefined;
};

...
export const generateLazy = <T>(ctx: GenContext<T>): unknown => {
  ...
  return ctx.generate(innerSchema as z.ZodType<T>);
};

export const generatePromise = <T>(ctx: GenContext<T>): Promise<unknown> =>
  Promise.resolve(ctx.generate(schemaDef<z.core.$ZodPromiseDef>(ctx.schema).innerType as z.ZodType<T>));

export const generatePipe = <T>(ctx: GenContext<T>): unknown =>
  ctx.generate(schemaDef<z.core.$ZodPipeDef>(ctx.schema).in as z.ZodType<T>);

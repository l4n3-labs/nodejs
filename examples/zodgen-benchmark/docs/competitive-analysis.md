...
This analysis compares zodgen against 5 open-source Zod mock data generators across features, API design, maintenance status, performance, and ecosystem fit. Data was gathered from GitHub repos, npm registries, programmatic feature testing, and Vitest benchmarks.

**Key change since last analysis:** zodgen shipped semantic field detection, locale API, invalid data generation, recursive depth-limiting, and 8 additional string formats. Three of five competitors (zod-fixture, zod-mocking, zodock) are now **broken on Zod v4** — they fail on import. The competitive field has narrowed to **three viable tools**: zodgen, zod-schema-faker, and zocker.
...
| Schema Complexity | zodgen | zod-schema-faker | zocker | Winner |
|-------------------|--------|------------------|--------|--------|
| Primitive (string, number, bool) | 62,377 | **851,982** | 38,172 | zod-schema-faker (13.7x) |
| Simple object (4 fields) | **55,748** | 410 | 32,921 | zodgen (136x vs zsf, 1.7x vs zocker) |
| Constrained object (uuid, email, ranges) | **34,718** | 399 | 2,497 | zodgen (87x vs zsf, 13.9x vs zocker) |
| Nested object (enums, arrays, optional) | **33,218** | 439 | 8,576 | zodgen (76x vs zsf, 3.9x vs zocker) |
| Recursive tree (z.lazy) | **23,136** | 4,565 | 349 | zodgen (5.1x vs zsf, 66x vs zocker) |

### Batch Generation (ops/sec, higher is better)

| Batch Size | zodgen | zod-schema-faker | zocker | zodgen Advantage |
|------------|--------|------------------|--------|------------------|
...
- **zodgen** dominates every real-world schema by **4x–627x** over competitors. The advantage grows with schema complexity and batch size
- **zocker** is consistently 2nd but **3–52x slower** than zodgen, with the gap widening dramatically on constrained schemas
- Batch generation is where zodgen's architecture pays off most: at 1,000 objects, zodgen is **611x faster** than zod-schema-faker and **8.6x faster** than zocker
...
| | | **Slowest on real-world schemas** (87–627x slower than zodgen) |
| **External** | **Opportunities** | **Threats** |
| | Expand API with builder pattern, batch utilities | zodgen's performance and feature dominance |
| | Largest format catalog is a moat | zodgen closing the format gap |
...
| | | 3–52x slower than zodgen on benchmarks |
| **External** | **Opportunities** | **Threats** |
| | Expand override system | zodgen matches semantic detection + far richer API |
| | Bundle size reduction | zodgen's performance advantage grows with complexity |
...
| | | zodgen now offers invalid generation with active maintenance |
| **External** | **Opportunities** | **Threats** |
...
1. **Publish to npm** (Blocker)
   - zodgen's feature set now exceeds all competitors — distribution is the only gap
...
5. **Constraint-aware fuzz testing** (High effort, unique differentiator)
   ...
   - Leverages zodgen's deep constraint awareness + new invalid generation
...
- **Zero-dependency mode**: High effort, niche use case. zodgen's ~10 KB (excluding faker) is already reasonable.
...
zodgen has moved from a strong but incomplete contender to the **most feature-rich and performant** Zod mock data generator. With the Zod v4 migration eliminating 3 of 5 competitors from viability, and zodgen now matching or exceeding the unique advantages of the remaining two (zocker's semantic detection, zod-schema-faker's format breadth), the primary barrier to adoption is npm publication.

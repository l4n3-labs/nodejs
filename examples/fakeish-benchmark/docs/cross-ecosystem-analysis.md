...
**Date:** March 25, 2026
**Purpose:** Identify features from best-in-class mock data libraries across languages that could strengthen fakeish's roadmap

## Overview

With fakeish's Zod-specific competitive gaps largely closed (see [competitive-analysis.md](./competitive-analysis.md)), this analysis looks beyond the Zod/JS ecosystem to identify patterns and capabilities from 5 mature, well-maintained mock data libraries in other languages.
...
**Why it matters:** fakeish has `unique` constraints in batch generation but no concept of *ordering* or *sequential* values. Auto-incrementing IDs and monotonic timestamps are among the most common needs in test data. Currently users must use overrides with manual counters.
...
json-schema-faker exposes a `optionalsProbability` float (0–1). Bogus and Instancio let you control nullable probability per field or globally. fakeish currently hardcodes 80% present / 20% absent for optionals and 80% value / 20% null for nullables.
...
**Why it matters:** fakeish overrides receive `ctx` with path and schema info, but cannot reference *other generated fields on the same object*. This means you can't generate an email that matches the username, or a fullName that combines firstName and lastName. This is one of the most requested features across all factory libraries.
...
**Effort:** Medium — traits are named override bundles. Needs composition logic (merging multiple traits) and conflict resolution (last-wins or error). Fits naturally into fakeish's immutable builder pattern.
...
**Proposed API:**
```typescript
import { toSQL, toCSV, toJSONLines } from '@l4n3/fakeish/formats'
...
**Effort:** Medium — serializers are straightforward but need to handle nested objects, arrays, dates, and null values. Could be a separate package (`@l4n3/fakeish-formats`) to keep core bundle small.
...
**Why it could matter:** Test data often involves relationships (User has Posts, Post has Comments, Comment references User). Currently fakeish users must generate each entity separately and manually wire foreign keys, which is error-prone and verbose.
...
**Why it could matter:** fakeish has `.generator()` for per-instance overrides but no global registry or community plugin conventions. A plugin system enables ecosystem growth — domain-specific generators for geo, finance, healthcare, etc.

**Proposed concept:**
```typescript
import { registerPlugin } from '@l4n3/fakeish'
import geoPlugin from '@l4n3/fakeish-plugin-geo'

registerPlugin(geoPlugin)
// Now fixture() automatically recognizes latitude, longitude, polygon, bbox semantics
```

**Effort:** Medium — fakeish already has a generator registry. Needs a `registerPlugin()` entry point and conventions for semantic/format contributions.
...
**Recommendation:** Better suited as integration with an existing property-based testing library (e.g., `fast-check`) rather than reimplementation within fakeish. A `toArbitrary(schema)` adapter that converts Zod schemas to fast-check arbitraries would capture most of the value.
...
```typescript
import { weighted } from '@l4n3/fakeish'
